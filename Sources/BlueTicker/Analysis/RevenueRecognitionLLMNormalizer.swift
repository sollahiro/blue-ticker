...
enum RevenueRecognitionLLMNormalizer {
    ...
    - 合計・小計・連結合計を表す行（例: 「顧客との契約から生じる収益」「外部顧客への売上高」「合計」）は row_kind="subtotal" とし、除去・消去を表す行は row_kind="reconciling" とすること。純粋な製品・事業・部門区分の行は row_kind="segment" とすること
    - 該当する事業別・製品別・部門別データが候補テーブル群に存在しない場合は applicable=false を返すこと
    - notes フィールドに、表選択・転置有無の根拠を短く日本語で記すこと
    """
