...
enum BreakdownNormalizer {
    ...
    static func normalizeGoodwill(
        ...
        return buildCountBasisSnapshot(
            ...
            warningPrefix: "goodwill", labelsByTag: labelsByTag, memberParents: memberParents)
    }
    ...
    private static func normalizeSegmentMetric(
        ...
        let useEntityDenom = useConsolidatedEntityTotalAsDenominatorWhenPresent && total != nil
        return buildCountBasisSnapshot(
            perMember: perMember, amountTag: amountTag, total: total, axis: axis,
            warningPrefix: warningPrefix, labelsByTag: labelsByTag, memberParents: memberParents,
    ...
    static func normalizeSegmentAssets(
        ...
        guard let snapshot = normalizeSegmentMetric(
    ...
    /// 親を分割しきれない presentation 子（有報の「うち」）を `subtotal` へ落とす。
    ...
    private static func demotePartialNestedChildren(
        ...
        for child in amounts.keys where kinds[child] == "segment" {
            guard let parent = memberParents[child], kinds[parent] == "segment",
                  amounts[parent] != nil
            else { continue }
            childrenByParent[parent, default: []].append(child)
        }
        for (parent, children) in childrenByParent {
            let parentAmount = amounts[parent]!
            guard parentAmount > 0 else { continue }
            let childSum = children.reduce(0.0) { $0 + amounts[$1]! }
            guard childSum > 0, childSum < parentAmount else { continue }
    ...
    /// その member を `subtotal` にする（三井住友トラスト S100O9U5 の銀行 member＝合計列）。
    /// 花王型の ±5% 一意除外は、残りが未タグ欠測で 5% 超不足すると発火しない。
    private static func demoteTaggedTotalEqualToDenominator(
        kinds: inout [String: String], amounts: [String: Double], total: Double?
    ) {
        guard let total, total > 0 else { return }
        let candidates = amounts.keys.filter { kinds[$0] == "segment" }.filter { member in
            amountsTightlyEqual(amounts[member] ?? 0, total)
        }
        guard candidates.count == 1, let taggedTotal = candidates.first else { return }
        let othersPositive = amounts.keys.contains { member in
            member != taggedTotal
                && (kinds[member] == "segment" || kinds[member] == "reconciling")
                && (amounts[member] ?? 0) > 0
        }
    ...
    private static func hasTaggedTotalEqualToDenominator(
        kinds: [String: String], amounts: [String: Double], total: Double
    ) -> Bool {
        amounts.contains { member, amount in
            kinds[member] == "subtotal" && amountsTightlyEqual(amount, total)
        }
    }

    private static func amountsTightlyEqual(_ a: Double, _ b: Double) -> Bool {
    ...
    static func finalizeSegmentAssetsSnapshot(_ snapshot: BreakdownSnapshot) -> BreakdownSnapshot {
        ...
        var warnings = snapshot.warnings.filter {
            $0 != "segment_assets_entity_total_differs_from_table_total"
                && $0 != "segment_assets_segment_sum_far_from_total"
        }

        let denominator: Double
        if let entity = entityAmount, entity > 0 {
            denominator = entity
            let scale = max(1.0, abs(entity), abs(reconciledSum))
            if reconciledSum > 0, abs(reconciledSum - entity) / scale > 0.05 {
                warnings.append("segment_assets_segment_sum_far_from_total")
            }
        } else if reconciledSum > 0 {
            denominator = reconciledSum
            if let entity = entityAmount, entity > 0 {
        ...
        let resolvedRows = rows.map { row -> BreakdownRow in
            var copy = row
        ...
        return BreakdownSnapshot(
            ...
            rows: resolvedRows.sorted { $0.labelRaw < $1.labelRaw }, sourceKind: snapshot.sourceKind,
            needsReview: !warnings.isEmpty, warnings: warnings)
    }
