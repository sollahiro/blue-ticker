...
struct RemoteAPIClient: Sendable {
    ...
    /// Waterfall（`docs/feature-tiers.md`）。financials の水準値に加え、増減分解フィールド
    ...
    func getWaterfall(code: String, years: Int) async -> RemoteOutcome<FinancialsResponse> {
        await getDecoding("/v1/companies/\(escape(code))/waterfall", query: ["years": String(years)])
    }
