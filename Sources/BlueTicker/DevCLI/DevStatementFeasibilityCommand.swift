...
struct DevStatementFeasibilityCommand: AsyncParsableCommand {
    ...
    static func debugPolicyHoldingSecurities(docID: String, xbrlDir: URL) {
        ...
        case .resolved(let payload, let source, let contentHash):
            ...
            if let summary = payload.policyHoldingSummary {
                print(
                    "  summary: unlisted=\(summary.unlistedIssueCount.map { String($0) } ?? "-")件/\(summary.unlistedCarryingAmount.map { String($0) } ?? "-")円 listed=\(summary.listedIssueCount.map { String($0) } ?? "-")件/\(summary.listedCarryingAmount.map { String($0) } ?? "-")円"
                )
            }
        case .notApplicable(let reason):
            print("notApplicable(\(reason))")
