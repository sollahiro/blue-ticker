...
- **異常値候補スキャン**: 分析キャッシュを整合性チェックし、目視レビュー用の候補を列挙
...
```bash
ticker cache status
ticker summarize 7203
ticker summarize 7203 --years 6
ticker summarize 7203 --half
ticker summarize 7203 --no-cache
```
...
分析キャッシュを整合性チェック（BS恒等式・フリーCF恒等式・ウォーターフォール分解残差・前年比急変など）にかけ、目視レビューに値する候補を列挙します。最終判断は人間が行う前提のレビュー支援機能です。

```bash
ticker inspect              # キャッシュ済み全銘柄を走査
ticker inspect 7203 6758    # 指定銘柄のみ
ticker inspect --format table
```

JSON 出力の `evidence` フィールドに判断根拠の値が含まれます。
