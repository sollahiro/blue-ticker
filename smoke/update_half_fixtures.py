"""
smoke/update_half_fixtures.py — smoke_half_expected/*.json を実際の抽出値で更新する（半期報告書）。

ロジック変更により値が意図的に変わった場合に実行し、フィクスチャを更新する。
更新後は .checksums.json も自動的に更新される。

実行方法:
    poetry run python smoke/update_half_fixtures.py --ticker 2802         # 1社のみ
    poetry run python smoke/update_half_fixtures.py --ticker 2802 8316   # 複数指定
    poetry run python smoke/update_half_fixtures.py --all                 # 全件（明示的に一括更新）
"""
import argparse
import asyncio
import hashlib
...
def _save_checksums(checksums: dict[str, str]) -> None:
    out: dict[str, Any] = {
        "_note": "SHA-256 of each fixture file. Managed by update_half_fixtures.py — do not edit manually.",
    }
...
async def _main() -> None:
    parser = argparse.ArgumentParser(description="update half-year smoke fixtures")
    ...
    for fixture_path in sorted(f for f in HALF_FIXTURE_DIR.glob("*.json") if not f.name.startswith(".")):
        ...
        new_fixture: dict[str, Any] = {
            "_comment": "半期報告書。金額は円単位。effective_tax_rate は比率（例: 0.254 = 25.4%）。CFは半期で省略される場合があり null になることがある。",
            "code": code,
            "name": name,
            "fy_end": fy_end,
