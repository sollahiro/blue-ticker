"""
smoke/check_half.py — smoke_half_expected/*.json と実際の抽出値を比較し、差分を検出する（半期報告書）。

本番ロジックを変更した後に実行し、意図しない値の変化がないかを確認する。
差分がある場合は exit(1)。

フィクスチャを直接編集するとチェックサム検証で弾かれる。
更新は update_half_fixtures.py --ticker XXXX で行う。

実行方法:
    poetry run python smoke/check_half.py           # 通常チェック
    poetry run python smoke/check_half.py --verbose  # 差分発生時にコンポーネント詳細を表示
"""
import argparse
import asyncio
import hashlib
...
from _common import (
    HALF_FIXTURE_DIR,
    edinet_cache_dir,
    extract_half_actuals,
    has_any_expected_value,
    load_half_fixtures,
)

_CHECKSUM_FILE = HALF_FIXTURE_DIR / ".checksums.json"
...
def _verify_checksums() -> list[str]:
    if not _CHECKSUM_FILE.exists():
        return [".checksums.json が見つかりません — smoke/smoke_half_expected/.checksums.json を生成してください"]

    recorded: dict[str, str] = json.loads(_CHECKSUM_FILE.read_text(encoding="utf-8"))
    violations: list[str] = []

    for fixture_path in sorted(f for f in HALF_FIXTURE_DIR.glob("*.json") if not f.name.startswith(".")):
        expected_hash = recorded.get(fixture_path.name)
        if expected_hash is None:
            violations.append(f"{fixture_path.name}: チェックサム未登録（update_half_fixtures.py --ticker で追加してください）")
            continue
...
async def _main() -> int:
    parser = argparse.ArgumentParser(description="half-year smoke check")
    ...
    if violations:
        ...
        print("  → フィクスチャの更新は update_half_fixtures.py --ticker XXXX で行ってください", file=sys.stderr)
        return 1
    ...
    if not fixtures:
        print("smoke_half_expected/ にフィクスチャファイルがありません", file=sys.stderr)
        return 1
    ...
    for fixture_id, fixture in fixtures:
        ...
        actuals = await extract_half_actuals(code, fy_end, cache_dir)
        if actuals is None:
    ...
    if total_compared == 0:
        ...
        return 1
    if total_diffs > 0:
        print(f"\n{total_diffs} 件の差分があります。意図的な変更なら update_half_fixtures.py を実行してください。")
        return 1
    print("\n全フィクスチャ一致。")
    return 0
