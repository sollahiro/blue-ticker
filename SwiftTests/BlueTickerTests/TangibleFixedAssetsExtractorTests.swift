...
final class TangibleFixedAssetsExtractorTests: XCTestCase {
    ...
    func testAjinomotoValues() {
        ...
        XCTAssertEqual(result.total, 581_330 * Financial.millionYen)
    }

    func testHitachiValues() {
        let result = extract("""
        ...
        """)
        XCTAssertEqual(result.accountingStandard, "IFRS")
        XCTAssertEqual(result.total, 1_341_537 * Financial.millionYen)
    }

    func testPriorYearValues() {
    ...
    func testDnpValues() {
        ...
        XCTAssertEqual(result.method, "field_parser")
        XCTAssertEqual(result.total, 405_795 * Financial.millionYen)
    }

    // MARK: - 取得原価 − 累計減価償却フォールバック（トヨタ相当）

    func testToyotaValues() {
        ...
        XCTAssertEqual(result.accountingStandard, "IFRS")
        XCTAssertEqual(result.method, "field_parser")
        XCTAssertEqual(result.total, 15_333_693 * Financial.millionYen)
    }

    func testTotalFallbackToCostCalculation() {
        ...
        """)
        XCTAssertEqual(result.method, "field_parser")
        XCTAssertEqual(result.total, (33_867_518 - 18_533_826) * Financial.millionYen)
    }

    // MARK: - not_found

    func testReturnsNotFoundWhenNoPpeTags() {
        let result = extract("""
