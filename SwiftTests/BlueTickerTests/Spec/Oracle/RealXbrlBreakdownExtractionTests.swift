...
@Suite struct RealXbrlBreakdownExtractionTests {
    ...
    @Test func pillarGeographyDropsOfWhichChinaAndLabelsPriorThenCurrent() async throws {
        ...
        #expect(result.method == "html_table")
        let prior = try #require(result.tables.first {
            containsAmount($0.markdown, "40,762") && containsAmount($0.markdown, "57,988")
        })
        let current = try #require(result.tables.first {
            containsAmount($0.markdown, "38,840") && containsAmount($0.markdown, "59,479")
        })
        #expect(prior.period == "前期")
        #expect(current.period == "当期")
