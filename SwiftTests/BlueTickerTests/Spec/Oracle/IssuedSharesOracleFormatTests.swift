...
//
// `issued_shares` は 2026-08-11 に as_of_period_end（離散タグ）＋ events（textblock表）の二段へ拡張。
...
@Suite struct IssuedSharesOracleFormatTests {
    private static let expectedFileURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("smoke/statement_notes_issued_shares_expected.json")

    private func assertMatchesOracle(docID: String, xbrlDir: URL) throws {
        let result = StatementNotesResolver.resolveIssuedShares(xbrlDir: xbrlDir)
