...
private func fakeSnapshot(
    axis: String = "business", sourceKind: String = "html_table", needsReview: Bool = false
...
@Suite struct CompanySegmentBreakdownTests {
    ...
    @Test func createAndFindRoundTripsPayloadAndOptionalLLMAudit() async throws {
        try await withMigratedApp { app in
            ...
            #expect(found.source == "html_table_llm")
    ...
    @Test func sameDocIDDifferentAxisCoexistAsSeparateRows() async throws {
        try await withMigratedApp { app in
            ...
            business.payload = fakeSnapshot(axis: "business", sourceKind: "html_table")
            ...
            geography.source = segmentBreakdownSourceHtmlTableLLM
    ...
    @Test func needsReviewColumnIsQueryableWithoutTouchingPayload() async throws {
        try await withMigratedApp { app in
            ...
            flagged.payload = fakeSnapshot(sourceKind: "html_table", needsReview: true)
            flagged.needsReview = true
            flagged.source = segmentBreakdownSourceHtmlTableLLM
