...
- 新規 REST 応答は `Codable` / `Sendable` な専用型と `CodingKeys` で定義する。欠落キーと `null` は区別し、必須 nullable キーは `encode(_:forKey:)` / `encodeNil` で明示する。既存応答の移行は `Spec/Contract` にキー・型・null・射影の期待値を先に固定する（`docs/api-compatibility.md`）。
