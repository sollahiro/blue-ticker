...
**必ず実 GET**、マジックバイト。Content-Type は信用しない。WP 既定 `w-logo-*-white-bg.png` は棄てる。フッター SNS（Facebook 等）、社名検索で当たった別法人ホスト、ページ全体の OGP スクリーンショットも棄てる。持株 vs 子会社は本体マーク（8377 はほくほく FG。北海道銀行禁止）。例外: 持株 16×16 のときだけグループ／証券の大きい公式画像。
