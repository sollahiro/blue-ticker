...
複数銘柄を分析した後や、数値に違和感があるときは、分析キャッシュを整合性チェックにかける。

```bash
ticker inspect              # キャッシュ済み全銘柄
ticker inspect <code>       # 指定銘柄のみ
```

候補はあくまでレビュー対象であり、異常の確定ではない。`severity` が `warning` のもの（恒等式不一致・前年比急変など）を優先して、`evidence` の値と有報原文を突き合わせて判断する。
