...
description: Neon を使った disposable 検証、本番 ingest、訂正有報の手動処理を行うときに使う。
---
...
- 本番で `DROP`、探索 ingest、未検証ロジックの書き込みを行わない。
...
5. 件数とゲートの現在地は `.agents/skills/tracker/SKILL.md` に従って Linear へ記録する。

## Disposable の 42P07

テーブルが存在し `_fluent_migrations` が空の場合だけ、接続先が disposable でデータが空であることを確認して schema を再作成する。本番には適用しない。
