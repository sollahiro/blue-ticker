...
- **テスト**: 仕様＝振る舞いを検証する。境界値・異常系を重視し、呼び出し順や内部構造は見ない。golden回帰（`RealXbrl*Tests.swift`）とsmoke（`SmokeTests.swift`）は目的が別（詳細は `docs/xbrl-parsing.md` §6）: goldenはロジック最適化の過程で見つけたエッジケース企業をその都度追加する蓄積型、smokeは会計基準・決算期変更等の境界を固定企業セットで横断確認する。対象企業は必ずしも一致しないため、goldenで検証済みの企業をsmokeへ昇格させることも都度検討する。
...
| # | 段階 | 書き込み先 | バンプ |
|---|---|---|---|
| 1 | 実装初期は書類30件分を使い捨てNeonで確認し、ロジックをある程度ブラッシュアップしてから225へ進む | 使い捨て | しない |
| 2 | 日経225限定で本番へ初期投入（最新年度を埋める。探索的試し書き禁止） | 本番 write | しない |
| 3 | 最新年度 100% 後にロジック改善（母数＝最新有報が取れた社。欠測は正当か不具合か確認） | — | しない |
| 4 | 改善結果を使い捨てで検証 | 使い捨て | しない |
| 5 | 問題なければ公開（都度確認）し、225 **全件**を本番へ揃える | 本番 write | しない |
| 6 | 全件を見たうえでのロジック改善 | — | しない |
| 7 | 不審フラグ（`needs_review`・あいまい失敗・異常欠測など）は手動 ingest | 本番 write | しない |
| 8 | 225 全体で問題なければ全銘柄へ拡張 | 本番 write | しない |
| 9 | 全銘柄展開に伴うロジック定着 | 本番 write | **する** |
