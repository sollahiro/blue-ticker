...
- [~] statement-notes: 実装済み・main反映済み（PR #172、2026-08-02）。note_type 9種を実装・配信（決定論のみ、REST `GET /v1/companies/{code}/statement/notes`・MCP `get_statement_notes`）。dividends/per_share_information/capital_expenditures_overview/issued_sharesは注記からXBRL直接抽出（決議・イベント単位のテーブル等）、research_and_developmentのみ財務取り込み単一値passthrough。treasury_stock_acquisitionはnote_typeごと廃止（自己株式取得は将来Statement本体側で持分変動計算書から取得する方針）。sga_breakdownは実装済みだが配信見送り（非連結のみタグ化されるためユーザー判断で保留、resolver・テストは残置）。property_plant_equipment_schedule/goodwill_and_intangiblesはIFRS連結企業限定でJ-GAAP単体附属明細表は未対応。政策保有株式は決定論実装（実データ4社・181銘柄をgolden化）。副産物としてbreakdownsに`employees`/`research_and_development`軸を追加（実装済み）。**残**: borrowings_schedule_cf_supplementは実データレビュー完了（日経225全224銘柄・単年度、`BLUE_TICKER_USER_DATA_PATH`分離キャッシュで測定）。**213/224解決（notApplicable 11）**。J-GAAP附属明細表・IFRS注記の多数のフォールバック経路（見出しアンカー・満期構成ペアテーブル・自社拡張タグ各種・列見出し表記ゆれ・複数表の合算・リース専用注記単独行対応等）を実装。設計方針（2026-08-04、ユーザー承認）: 通常経路（`parseComparisonTable`）はラベルで絞り込まず注記の全行をそのまま構造化し注記自身の「合計」行を採用する。ただし三井物産のみ例外的に、注記側のラベル品質が悪い（区分見出しと金額行が分離）ため連結BSの直接数値タグ（`parseDirectDebtFacts`、3タグ揃う場合のみ）を優先する。golden test 10件（丸紅・日立・ソニーグループ・三菱重工業・東京海上HD・住友金属鉱山・三井物産・ファーストリテイリング・ディスコ・中外製薬）。残notApplicable11件: 米国会計基準6社（対応見送り、キヤノン・野村HD・小松製作所・オリックス・オムロン・富士フイルムHD）／注記自体に明細が存在しない4社（キーエンス・ネクソン・トレンドマイクロ・ARCHION、いずれも附属明細表に「該当事項はありません」等で確認済み）／トヨタの日付分割ロールフォワード1社。内訳確認済み（11/11）。property_plant_equipment_schedule/goodwill_and_intangiblesの実データ目視レビューは未着手。cache_versionは未公開のためバンプ見送り。breakdowns employees/research_and_development軸の公開可否の正式確認も残タスク。標準タクソノミラベル（`assets/taxonomy`、約105MB）を本番にバンドルするかは別途要検討（現状PPE/のれん明細等の標準タグはラベルnullのまま配信）。将来構想（Allocation、配分構造のサンキー図可視化）は `docs/allocation-concept.md` 参照
