...
- [~] statement-notes: 実装済み・main反映済み（PR #172、2026-08-02）。note_type 9種を実装・配信（決定論のみ、REST `GET /v1/companies/{code}/statement/notes`・MCP `get_statement_notes`）。dividends/per_share_information/capital_expenditures_overview/issued_sharesは注記からXBRL直接抽出（決議・イベント単位のテーブル等）、research_and_developmentのみ財務取り込み単一値passthrough。treasury_stock_acquisitionはnote_typeごと廃止（自己株式取得は将来Statement本体側で持分変動計算書から取得する方針）。sga_breakdownは実装済みだが配信見送り（非連結のみタグ化されるためユーザー判断で保留、resolver・テストは残置）。property_plant_equipment_schedule/goodwill_and_intangiblesはIFRS連結企業限定でJ-GAAP単体附属明細表は未対応。政策保有株式は決定論実装（実データ4社・181銘柄をgolden化）。副産物としてbreakdownsに`employees`/`research_and_development`軸を追加（実装済み）。**残**: borrowings_schedule_cf_supplementは実データレビュー完了（日経225全224銘柄・単年度、`BLUE_TICKER_USER_DATA_PATH`分離キャッシュで測定）。**211/224解決（notApplicable 13）**。J-GAAP附属明細表・IFRS注記の各種フォールバック（見出しアンカー方式、満期構成ペアテーブル、自社拡張タグ＋rowspan補正、列見出し表記ゆれ「帳簿残高」、セル分割見出し「帳簿 価額」、日付が装飾行を挟んだ2行目ヘッダーに埋め込まれるケース）に加え、三菱重工業・東京海上HDの自社拡張タグ、標準タグ`NotesOtherFinancialLiabilitiesConsolidatedFinancialStatementsIFRSTextBlock`（住友金属鉱山で発見。副次的に本田技研工業・協和キリンも解決）、三井物産（短期／長期債務が別表に分かれる自社拡張タグ＋2段ヘッダー列ズレ補正）、ファーストリテイリング（資産/負債併存タグから負債側のみ選択する条件を追加）に対応。**三井物産のみ例外的に、注記のHTML明細表より連結BSの直接数値タグ（`ShortTermDebtCLIFRS`／`CurrentPortionOfLongTermDebtCLIFRS`／`LongTermDebtNCLIFRS`、3タグとも揃う場合のみ）を優先する`parseDirectDebtFacts`を新設**（注記側は区分見出し行と金額行が分離しラベル品質が悪いため）。golden test 8件（丸紅・日立・ソニーグループ・三菱重工業・東京海上HD・住友金属鉱山・三井物産・ファーストリテイリング）。**設計方針（2026-08-04、ユーザー承認）**: `parseComparisonTable`はラベルでの絞り込みを行わず注記の全行をそのまま構造化し、注記自身の「合計」行を採用する（社債・借入金・リース以外の項目、例: 三菱重工業のデリバティブ負債・債権流動化に伴う支払債務、東京海上の投資契約負債、住友金属鉱山のデリバティブ負債、も注記どおり含む）。直接タグが揃う場合はそちらを優先（ラベル品質・正確性が上）。残notApplicable13件の内訳: 米国会計基準6社（対応見送り、キヤノン・野村HD・小松製作所・オリックス・オムロン・富士フイルムHD）／注記自体に明細が存在しない6社（キーエンス・ディスコ・ネクソン・トレンドマイクロ・ARCHION=附属明細表に「該当事項はありません」と明記、中外製薬=関連タグ自体が存在しない）／トヨタの日付分割ロールフォワード1社。**内訳確認済み（13/13）**。property_plant_equipment_schedule/goodwill_and_intangiblesの実データ目視レビューは未着手。cache_versionは未公開のためバンプ見送り。breakdowns employees/research_and_development軸の公開可否の正式確認も残タスク。標準タクソノミラベル（`assets/taxonomy`、約105MB）を本番にバンドルするかは別途要検討（現状PPE/のれん明細等の標準タグはラベルnullのまま配信）。将来構想（Allocation、配分構造のサンキー図可視化）は `docs/allocation-concept.md` 参照
