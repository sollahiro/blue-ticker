...
| statements | 日経225限定。DB/ingest/REST/MCP 配線済み（`statement-v1`）。**BS/PL/CF/SS**（SS=合計列のみ・`order` 付き・期首→変動→期末）。区分(`section`)・`is_total`/`components` 対応。US-GAAP は `notApplicable`。golden: トヨタ等＋smoke 9社（BS/PL/CF と SS）。PL 利益段階ラベリングはスコープ外。**本番 Neon の `company_statements` は 0 行**（日経225全社 ingest 未実施） |
