...
- [ ] 抽出ロジック変更時の差分検証ツール
- [ ] LLM による抽出値の抜き打ち整合評価
- [~] Stage 7/8: Statement（財務諸表 BS/PL/CF 完全正規化＋注記）。抽出ロジック・DevCLI は実装済み（PR #153）。DB/ingest/REST/MCP 配線に着手（2026-07-29、対象は日経225限定でスタート。実装方針は `docs/statement-normalization-concept.md`「実装方針」参照）
