...
- [~] statement-notes: 実装済み・main反映済み（PR #172、2026-08-02）。note_type 9種を実装・REST/MCP配線済み（決定論のみ、`GET /v1/companies/{code}/statement/notes`・MCP `get_statement_notes`）。**ただし本番Neonの`company_statement_notes`は2026-08-08時点で0行**（`company_statements`も同様に0行）。statementsと同じく日経225全社への本番ingestが未実施のため、以下の「実データレビュー」「golden test」はいずれも本番ではなくDevCLIのローカルキャッシュ走査（`--export-notes-review`等）によるオフライン検証結果であり、実際にAPIから配信されているデータではない点に注意。dividends/per_share_information/capital_expenditures_overview/issued_sharesは注記からXBRL直接抽出（決議・イベント単位のテーブル等）、research_and_developmentのみ財務取り込み単一値passthrough。treasury_stock_acquisitionはnote_typeごと廃止（自己株式取得は将来Statement本体側で持分変動計算書から取得する方針）。sga_breakdownは実装済みだが配信見送り（非連結のみタグ化されるためユーザー判断で保留、resolver・テストは残置）。property_plant_equipment_schedule/goodwill_and_intangiblesはIFRS連結企業限定でJ-GAAP単体附属明細表は未対応。政策保有株式は決定論実装（実データ4社・181銘柄をgolden化）。副産物としてbreakdownsに`employees`/`research_and_development`軸を追加（実装済み）。note_type `borrowings_schedule_cf_supplement` は2026-08-08に `borrowings_schedule` へリネーム（cache_versionはバンプ不要。公開API値のため本番DBのUPDATE移行を検討したが、上記のとおり本番0行のため移行自体が不要と判明）。**残**: borrowings_scheduleは実データレビュー完了（日経225全224銘柄・単年度、`BLUE_TICKER_USER_DATA_PATH`分離キャッシュで測定）。**214/224解決（notApplicable 10）**。J-GAAP附属明細表・IFRS注記の多数のフォールバック経路（見出しアンカー・満期構成ペアテーブル・自社拡張タグ各種・列見出し表記ゆれ・複数表の合算・リース専用注記単独行対応・ロールフォワード表の期末残高列抽出等）を実装。設計方針（2026-08-04、ユーザー承認）: 通常経路（`parseComparisonTable`）はラベルで絞り込まず注記の全行をそのまま構造化し注記自身の「合計」行を採用する。ただし三井物産のみ例外的に、注記側のラベル品質が悪い（区分見出しと金額行が分離）ため連結BSの直接数値タグ（`parseDirectDebtFacts`、3タグ揃う場合のみ）を優先する。トヨタは`有利子負債`注記が前/当比較列を持たずロールフォワード表（期首残高｜CF内訳｜期末残高）2つで構成されるため専用パーサー（`parseRollforwardEndingBalancePairTables`、各行の最終セル＝期末残高を読む）で解決。golden test は当初14件（丸紅・日立・ソニーグループ・三菱重工業・東京海上HD・住友金属鉱山・三井物産・ファーストリテイリング・ディスコ・中外製薬・トヨタ自動車・東京電力HD・三菱地所・HOYA）。**smoke企業11社での借入金突合（2026-08-08、PR #182）**で追加バグ4件を修正しgoldenを拡張: padding-left/ptによる銀行カテゴリ小計除外（三菱UFJ）、bare「計」＋流動/非流動並記の二重計上防止（クボタ）、値なし区分見出し消失による直前行欠落防止（ニチレイ）、単体版附属明細表タグへのフォールバック（東邦レマック）。あわせて整合確認済み4社（味の素・AZplanning・オークマ・スズキ）もgolden化。**リース債務の扱い（2026-08-08、ユーザー確認）**: IFRSで借入金注記とリース注記が分離している会社（味の素・スズキ・クボタ等）について、`borrowings_schedule` note_type へ別注記のリースを合流させるのは別スコープ。有利子負債（IBD）側は既存の `field_parser+lease_textblock`（`IFRSLease.extractLeaseLiabilities`）で既に加算済みであることをsmoke期待値で確認済み。残notApplicable10件: 米国会計基準6社（対応見送り、キヤノン・野村HD・小松製作所・オリックス・オムロン・富士フイルムHD）／注記自体に明細が存在しない4社（キーエンス・ネクソン・トレンドマイクロ・ARCHION、いずれも附属明細表に「該当事項はありません」等で確認済み）。内訳確認済み（10/10）。**監査レビュー（2026-08-05、Opusサブエージェント）で確定バグ3件を検出・修正**: (a) J-GAAP附属明細表で同一セル内縦積み行（カテゴリ+内訳）の判定が、直前の無関係な通常行を誤って巻き込み消していた（東京電力の短期借入金2.9兆円等が欠落）。(b) 列幅による折り返し（`<p>`2つに分割されるだけの1ラベル）を縦積み複数項目と誤認識しラベル切り詰め＋行欠落（三菱地所）。(c) J-GAAP経路が「小計」区分を除外せず二重計上（三菱地所・三井不動産）。加えて監査後の自己検証で新規リグレッション1件を発見・修正: 三井物産向けに実装した複数テーブル合算ロジックが、HOYAでは無関係なロールフォワード表まで合算し三重計上・マイナス値混入を起こしていた（行ラベル重複時は合算しないガードを追加）。cache_versionをバンプ（`companyFinancialsCacheVersion` fin-v4→fin-v5、`borrowingsScheduleNoteCacheVersion` v1→v2）。母集団全体（224社）の異常値機械スキャンで残る9件の表示ラベル重複（例: 大塚HDの担保付/無担保短期借入金が同一ラベルに収束）は既知の限界として許容（値・合計は正しい、区分見出しがラベルに引き継がれないだけ）。property_plant_equipment_schedule/goodwill_and_intangiblesの実データ目視レビューは未着手。breakdowns employees/research_and_development軸の公開可否の正式確認も残タスク。標準タクソノミラベル（`assets/taxonomy`、約105MB）を本番にバンドルするかは別途要検討（現状PPE/のれん明細等の標準タグはラベルnullのまま配信）。将来構想（Allocation、配分構造のサンキー図可視化）は `docs/allocation-concept.md` 参照
