...
- [~] **US-GAAP Statement HTML 経路**（2026-08-10 試作）。`USGAAPStatementHtml` で連結 HTML→行を決定論抽出（富士フイルム S100W3XJ / キヤノン S100XTLJ）。結果は要確認。取れない場合のみ `notApplicable(us_gaap_unsupported)`。`borrowings_schedule` は同 reason で対象外のまま。financials/IBD の `USGAAPHtml` は現行 summary 用として別。`statement-v1` のまま
