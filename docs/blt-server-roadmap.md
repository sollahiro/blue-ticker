...
- [~] Stage 6: 事業別・地域別売上の正規化（企業間比較用）。business 軸（日経225構成銘柄限定）は抽出・正規化・永続化・ingest(`--stages 6`)/REST(`segment-breakdown`)/MCP(`get_segment_breakdown`)まで実装済み（PR #87/#88/#91 + business軸配線）。銀行・保険の粗利益/営業純益基準、NTT等のタグ一般化、小松製作所の年度ラベルチェーン修正等を2026-07-21〜22に反映。**未着手**: geography 軸の ingest 配線、オリックス等の巨大単一USGAAP注記でのセグメント当期テーブル抽出（issue #103）、野村の金融費用控除後分母取り違え（issue #105）。構想と残タスクは `docs/segment-normalization-concept.md`「今後の検討事項」
- [ ] 抽出ロジック変更時の差分検証ツール
- [ ] LLM による抽出値の抜き打ち整合評価
