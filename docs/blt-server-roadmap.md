...
| financials / filing-sections | バックフィル継続。現行版・read 床は `versioning.md`。`fin-v6` は IBD の notes リース欠測埋め。Statement / Note / Breakdown → financials 組立完成は次世代 |
...
- Summary の次世代は **Statement / Note / Breakdown → financials** 組立。IBD の notes リース欠測埋めは `fin-v6`。組立完成の切替は次世代。Waterfall も同行走査のため同じ切替に乗る。
