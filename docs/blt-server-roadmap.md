...
- [~] Stage 6: 事業別・地域別売上の正規化（企業間比較用）。抽出・正規化ロジックと永続化スキーマ（`company_segment_breakdowns` / `segmentBreakdownCacheVersion`）は実装済み（PR #87/#88/#91）。**未着手**: `app.migrations` 登録・ingest/CLI/REST 配線。構想と残タスクは `docs/segment-normalization-concept.md`「今後の検討事項」
- [ ] 抽出ロジック変更時の差分検証ツール
- [ ] LLM による抽出値の抜き打ち整合評価
