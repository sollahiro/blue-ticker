...
- [~] statement-notes: `worktree-statement-summarize-feasibility` ブランチで実装中（未マージ）。note_type 9種を実装・配信（決定論のみ）。property_plant_equipment_schedule/goodwill_and_intangiblesはIFRS連結企業限定でJ-GAAP単体附属明細表は未対応。政策保有株式は当初LLM必須と想定していたが、実データ検証でEDINET標準タクソノミの銘柄別構造化タグと判明し決定論で実装（実データ4社・181銘柄をgolden化）。**sga_breakdown は実装済みだが配信見送り**（2026-08-02、実データレビューでXBRLタグ化されるのは常に非連結のみと判明、連結内訳は本文自由記述でタグ取得不可のためユーザー判断で保留。resolver・テストは将来の連結対応に備えて残置）。REST/MCPワイヤリングも完了（`GET /v1/companies/{code}/statement/notes`・MCP `get_statement_notes`）。副産物として breakdowns に`employees`/`research_and_development`軸を追加（実装済み・実データ未レビュー）。**dividends・per_share_informationは財務取り込み単一値passthroughからXBRL直接抽出へ再設計済み**（2026-08-02）。dividendsは注記の決議単位テーブル（`dividendEvents`、単一値だと配当イベントの内訳が失われると判明）。per_share_informationはEPS単一値からEPS/潜在株式調整後EPS/BPSの3指標（`items`）へ拡張（BPSはIFRS企業で誤タグ名`EquityToAssetRatioIFRSSummaryOfBusinessResults`経由、`unitRef=JPYPerShares`で判別。IFRS企業のEPSはStatement本体（損益計算書）と同一factで意図的重複）。capital_expenditures_overviewも同様に再設計済み（複数セグメント企業は注記HTMLのセグメント別テーブルをrowspan展開・内容ベース列判定でXBRL直接抽出、単一セグメント企業は総額タグへフォールバック。列数・ヘッダー文言・rowspanの使われ方の会社差を実データで吸収。コニカミノルタ等一部企業は複数セグメントが金額を共有する結合開示のため単純合計に注意）。issued_sharesも同様に再設計済み（発行済株式総数・資本金等の推移を決議・イベント単位で`issuedSharesEvents`としてXBRL直接抽出。株数=株/千株・金額=千円/百万円と単位が会社ごとに揺れるため生値へ正規化。行の採否は株数/資本金/資本準備金いずれかの増減欄が実数かで判定し、日立の期間ラベル行（変動なし確認用）だけ除外、JT・横河電機のように株数不変で資本準備金だけ動く行は取りこぼさない）。**treasury_stock_acquisitionはnote_typeごと廃止**（2026-08-02、自己株式の取得は将来Statement本体側で持分変動計算書から取得する方針。注記側の再設計・拡張は行わない）。将来構想（Allocation、配分構造のサンキー図可視化）は `docs/allocation-concept.md` 参照。詳細は `.claude/handoff.local.md`（このブランチのworktree限定・非追跡）参照
