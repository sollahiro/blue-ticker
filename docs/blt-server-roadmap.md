...
- [~] statement-notes: 実装済み・main反映済み（PR #172、2026-08-02）。note_type 9種を実装・配信（決定論のみ、REST `GET /v1/companies/{code}/statement/notes`・MCP `get_statement_notes`）。dividends/per_share_information/capital_expenditures_overview/issued_sharesは注記からXBRL直接抽出（決議・イベント単位のテーブル等）、research_and_developmentのみ財務取り込み単一値passthrough。treasury_stock_acquisitionはnote_typeごと廃止（自己株式取得は将来Statement本体側で持分変動計算書から取得する方針）。sga_breakdownは実装済みだが配信見送り（非連結のみタグ化されるためユーザー判断で保留、resolver・テストは残置）。property_plant_equipment_schedule/goodwill_and_intangiblesはIFRS連結企業限定でJ-GAAP単体附属明細表は未対応。政策保有株式は決定論実装（実データ4社・181銘柄をgolden化）。副産物としてbreakdownsに`employees`/`research_and_development`軸を追加（実装済み）。**残**: borrowings_schedule_cf_supplementは実データレビュー完了（日経225全224銘柄・単年度、`BLUE_TICKER_USER_DATA_PATH`分離キャッシュで測定）。**206/224解決（notApplicable 18）**。J-GAAP附属明細表・IFRS注記の各種フォールバック（見出しアンカー方式、満期構成ペアテーブル、自社拡張タグ＋rowspan補正、列見出し表記ゆれ「帳簿残高」、セル分割見出し「帳簿 価額」、日付が装飾行を挟んだ2行目ヘッダーに埋め込まれるケース）に加え、三菱重工業・東京海上HDの自社拡張タグ（それぞれ「社債、借入金及びその他の金融負債」「社債、借入金及び投資契約負債」）を`parseComparisonTable`にそのまま乗せて解決。golden test 5件（丸紅・日立・ソニーグループ・三菱重工業・東京海上HD）。**設計方針（2026-08-04、ユーザー承認）**: `parseComparisonTable`はラベルでの絞り込みを行わず注記の全行をそのまま構造化し、注記自身の「合計」行を採用する（社債・借入金・リース以外の項目、例: 三菱重工業のデリバティブ負債・債権流動化に伴う支払債務、東京海上の投資契約負債、も注記どおり含む）。Statement note_typeとして全体構造化を優先し、「有利子負債」等の特定タグのみの再集計は必要になった時点で別途行う方針。残notApplicable18件の内訳: 米国会計基準6社（対応見送り、キヤノン・野村HD・小松製作所・オリックス・オムロン・富士フイルムHD）／注記自体に明細が存在しない10社（キーエンス・ディスコ・ネクソン・トレンドマイクロ・ARCHION=附属明細表に「該当事項はありません」と明記、中外製薬・協和キリン=関連タグ自体が存在しない、三井物産・本田技研工業=注記が定性記述のみで表なし、ファーストリテイリング=前後期比較不可能な非対称開示）／トヨタの日付分割ロールフォワード1社／住友金属鉱山1社（公正価値注記が短期借入金/リース負債を欠く不完全な内訳のみで、自社拡張タグも存在しないため正確な合計を再現できない）。**内訳確認済み（18/18）**。property_plant_equipment_schedule/goodwill_and_intangiblesの実データ目視レビューは未着手。cache_versionは未公開のためバンプ見送り。breakdowns employees/research_and_development軸の公開可否の正式確認も残タスク。標準タクソノミラベル（`assets/taxonomy`、約105MB）を本番にバンドルするかは別途要検討（現状PPE/のれん明細等の標準タグはラベルnullのまま配信）。将来構想（Allocation、配分構造のサンキー図可視化）は `docs/allocation-concept.md` 参照
