...
- [~] statement-notes: 実装済み・main反映済み（PR #172、2026-08-02）。note_type 9種を実装・配信（決定論のみ、REST `GET /v1/companies/{code}/statement/notes`・MCP `get_statement_notes`）。dividends/per_share_information/capital_expenditures_overview/issued_sharesは注記からXBRL直接抽出（決議・イベント単位のテーブル等）、research_and_developmentのみ財務取り込み単一値passthrough。treasury_stock_acquisitionはnote_typeごと廃止（自己株式取得は将来Statement本体側で持分変動計算書から取得する方針）。sga_breakdownは実装済みだが配信見送り（非連結のみタグ化されるためユーザー判断で保留、resolver・テストは残置）。property_plant_equipment_schedule/goodwill_and_intangiblesはIFRS連結企業限定でJ-GAAP単体附属明細表は未対応。政策保有株式は決定論実装（実データ4社・181銘柄をgolden化）。副産物としてbreakdownsに`employees`/`research_and_development`軸を追加（実装済み）。**残**: borrowings_schedule_cf_supplementの実データレビューは完了（2026-08-03）。J-GAAP附属明細表のインデント二重計上・セル内複数行混在バグを修正、IFRS注記（3タグ＋日付ヘッダー対応）へのフォールバックを新設し日経225解決率79/144→128/144に改善。残16件はnotApplicableのまま据え置き: US-GAAP（全注記が1ブロックに束ねられ専用タグ非対応、2件・LLM化は母数比コスト見合わず見送り合意済み）、IFRS「金融商品に関する注記」が期間比較表を持たず満期バケット表のみの13件（当期首/当期末残高という現行契約に載せるには別の抽出設計が要るため保留）、トヨタ固有の日付分割フォーマット1件。property_plant_equipment_schedule/goodwill_and_intangiblesの実データ目視レビューは未着手のまま残る。cache_versionは未公開のためバンプ見送り。breakdowns employees/research_and_development軸の公開可否の正式確認も残タスク。標準タクソノミラベル（`assets/taxonomy`、約105MB）を本番にバンドルするかは別途要検討（現状PPE/のれん明細等の標準タグはラベルnullのまま配信）。将来構想（Allocation、配分構造のサンキー図可視化）は `docs/allocation-concept.md` 参照
