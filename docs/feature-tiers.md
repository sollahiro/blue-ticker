...
- **機能の課金境界**（無料 / 有料）はエンドポイント・ツール単位で決める（フィールド単位のマスキングではない）。同一機能の無料/有料は面によらず同じ。
- **面の課金・メーターは分けられること**を理想とする（REST として課金できる／MCP として課金できる）。認証・識別子も面ごとに分離できる設計を優先する（詳細は認証方式選定時に確定）。
