...
2. **caption** — 同一 TextBlock 内で表から直前へ遡る。単位行やラッパー `div` は飛ばし、直前の `<table>` と `【…】` の別注記見出しで止める。語: 前/当連結会計年度、前/当事業年度、前/当年度、前期/当期。`（自 YYYY年M月D日 至 YYYY年M月D日）` と「YYYY年度」は書類の `CurrentYearDuration` 期末と照合する。
