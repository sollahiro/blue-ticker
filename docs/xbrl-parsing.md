...
| Overview ライブ生成 | `SwiftTests/BlueTickerTests/Spec/Oracle/RealXbrlOverviewLiveTests.swift` | smoke固定11社（`SmokeTests` と同じ docID）。実 XBRL「事業の内容」→ 数字混入なし・だ・である調。50〜80字は目安（情報量が少なければ短くてよい。合格上限は90字）。文面は非決定なのでオラクル凍結しない。`BLT_OVERVIEW_LIVE=1` と `OPENROUTER_OVERVIEW_API_KEY` と `tmp_cache/edinet` が無いとき SKIP。格納は `company_overviews`（会社1社=1行、`overview-v3`。由来 doc_id は列）。「Nつの報告セグメントで〜」の枠まとめは不合格。ingest の再生成は、LLM 成功行は最新有報の doc_id 変更と `needs_review`（`cache_version` バンプでは動かない）。`not_applicable` は版ずれで再実行する。事業の内容が系統図のみで `applicable=false` のときは「報告セグメントの概要」（`DescriptionOfReportableSegmentsTextBlock`、無ければ見出しフォールバック）を再入力する。ingest stage は `overviews`。公開 REST は未配線 |
