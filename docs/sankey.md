...
| 開示 | 判定 | 親（L2 販管費）の値 | 子 |
|---|---|---|---|
| R&D が販管費の内数 | J-GAAP の `ResearchAndDevelopmentExpensesSGA` 等。IFRS で PL に R&D 行が無い（クボタ） | 連結 PL の販管費 | R&D（breakdown 分母）＋その他販管費 |
| R&D が PL 別掲 | PL に研究開発費行がある。キヤノン（US-GAAP HTML、販管費と兄弟）。味の素（IFRS、販売費・研究開発費・一般管理費が三兄弟） | **derived**＝開示の販売費＋一般管理費＋研究開発費。ラベルは「販売費及び一般管理費」。実タグは子に残す | 開示の販売費・一般管理費（まとめてその他販管費でも可）＋研究開発費 |

別掲でも親を販管費に寄せる。R&D を売上総利益の直下へ並べない。

- キヤノン: 親 `1,706,565,000,000` 円＝販管費 `1,367,277,000,000`＋R&D `339,288,000,000`。粗利益＝親＋営業利益。
- 味の素（S100VXJA）: 親 `397,775m`＝販売費 `211,976`＋一般管理費 `154,878`＋R&D `30,921`。Waterfall の `sga`（`366,854m`＝販売費＋一般管理費）は R&D を含まない。`resolveSGA` が IFRS 分割行を足すとき R&D タグを入れない。
...
└─ L1 売上総利益
   ...
   │  └─ L3 その他販管費
   └─ L2 営業利益                    ← statement の営業利益。Waterfall の事業利益とは別（下表）
      ├─ L3 営業外損益
      └─ L3 経常利益
         ├─ L4 特別損益
         └─ L4 税引前利益
...
#### L2 営業利益と Waterfall の事業利益

一致しない。名前も値も別物。Sankey の L2 は statement の営業利益。Waterfall の `business_profit` は `売上総利益 − 販管費`（販管費が無ければ `GP − OP` で販管費を埋めてから引く）。

| 例 | L2 営業利益 | Waterfall 事業利益 | 差の中身 |
|---|---|---|---|
| オークマ（J-GAAP） | 14,651m | GP − SGA ＝ 14,651m | 一致。R&D は販管費の内数 |
| 味の素（IFRS、S100VXJA） | 113,968m | 550,764 − 366,854 ＝ 183,910m | Waterfall の SGA は販売費＋一般管理費のみ。R&D 30,921m と持分法 6,314m とその他営業は入っていない |
| キヤノン（US-GAAP） | 455,390m | 2,161,955 − 1,367,277 ＝ 794,678m | 別掲 R&D 339,288m が販管費に入っていない |

金融機関は Waterfall が経常収益−販管費（ラベルが経常利益/事業利益のとき）。IFRS 金融の PL 行「事業利益」（`BusinessProfitIFRS*`）は statement OP のフォールバックであり、Waterfall の派生値ではない。Sankey は statement の行に従う。

味の素の開示 `BusinessProfitLossIFRS`（159,302m）は Waterfall の 183,910m と別物。開示の事業利益＝売上総利益＋持分法−販売費−研究開発費−一般管理費。Waterfall は持分法も R&D も引かない。

`売上総利益 →` Waterfall の `事業利益 → 営業利益` を独立した3段にはしない。

Waterfall の式 `事業利益 = 売上総利益 − 販管費` は、販管費を Waterfall の SGA に固定すれば常に成り立つ。その販管費から R&D を分解できるのは、R&D がその数の内数のときだけ。別掲 R&D はその販管費に入っていない。

| 開示 | Waterfall SGA の中身 | その販管費から R&D を分解 | Waterfall 事業利益 → OP に残るもの |
|---|---|---|---|
| 内数（オークマ） | 販管費合計（R&D 含む） | できる | 0。段が潰れる |
| 別掲（味の素） | 販売費＋一般管理費。R&D なし | できない | R&D 30,921m、持分法、その他営業。開示の事業利益 159,302m とも一致しない |
| 別掲（キヤノン） | 開示販管費。R&D なし | できない | R&D 339,288m |

キヤノン: 開示販管費 `1,367,277m`、別掲 R&D `339,288m`。`1,367,277 − 339,288` を「その他販管費」にすると、開示に無い残差を発明する。味の素も同じで、Waterfall SGA `366,854m` から R&D `30,921m` は出てこない。

別掲でも R&D を販管費の子にする（親＝販売費＋一般管理費＋R&D）と、キヤノンは GP − 親 ＝ OP になり、間の Waterfall 事業利益は営業利益と同一。Waterfall の販管費から分解することと、R&D を常に販管費の子にすることは両立しない。

開示の `BusinessProfitLossIFRS` がある会社（味の素）は statement に従う。derived 販管費のあと、その他営業を挟んで営業利益へ。Waterfall の派生値はノードにしない。

#### IFRS / US-GAAP

経常利益・特別損益を作らない。営業利益から金融損益・持分法（税引前の後に来る会社もある）・その他を経由して税引前へ。
販管費の下の R&D は上表どおり。別掲（キヤノン、味の素）でも親は derived 販管費。

味の素（S100VXJA）の開示カスケード:
...
│  ├─ 一般管理費 154,878
│  └─ 研究開発費 30,921
└─ 事業利益 159,302          ← BusinessProfitLossIFRS。Waterfall 事業利益ではない
   ├─ その他営業収益 4,936
   ├─ その他営業費用 50,269
   └─ 営業利益 113,968
```

クボタは PL に R&D 行も事業利益行も無く、販管費合計だけ。R&D は内数扱いで breakdown から販管費の子にする。
...
- 味の素: 売上総利益 → derived 販管費（販売費・一般管理費・別掲 R&D）・持分法 → 開示の事業利益 → その他営業 → 営業利益
