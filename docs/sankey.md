...
開示行は探さない。`business_profit = 売上総利益 − 販管費`（販管費が無ければ `GP − OP` で販管費を埋めてから引く）。売上総利益は売上高 − 売上原価であり、営業総利益（営業収益合計 − 営業原価合計）には置き換えない。味の素の `BusinessProfitLossIFRS`（159,302m）はこれではない。
