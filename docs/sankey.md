...
開示行は探さない。`business_profit = 売上総利益 − 販管費`（販管費が無ければ `GP − OP` で販管費を埋めてから引く）。売上総利益と営業総利益が両方あるとき（イオン・セブン＆アイ）は販管費の直上である営業総利益を GP にする。味の素の `BusinessProfitLossIFRS`（159,302m）はこれではない。
