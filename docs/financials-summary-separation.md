...
| `fin-vN` | 存続。IBD の notes リース欠測埋めは `fin-v6`。IA 切替（値の意味が変わらない配線）ではバンプしない。Extractor の符号・抽出意味が変わったら上げる（現行 `fin-v8`: IFRS `TotalNetRevenuesIFRS` を売上候補に追加。`fin-v7`: US-GAAP 自己株式取得の絶対値） |
