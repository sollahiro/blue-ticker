...
| `fin-vN` | 存続。IBD の notes リース欠測埋めは `fin-v6`。IA 切替（値の意味が変わらない配線）ではバンプしない。Extractor の符号・抽出意味が変わったら上げる（現行 `fin-v12`: IFRS 本表 `OperatingRevenuesIFRS`（営業収益）を Summary 売上に採用（NTT）。US-GAAP も単独「営業収益」を exact で採用。`fin-v11`: US-GAAP Summary 純資産が単独ラベル「資本合計」も採用。`fin-v10`: US-GAAP Summary 売上が中間の「〜売上高」より営業収益計 / 収益合計を優先。`fin-v9`: Summary に BPS。`fin-v8`: IFRS `TotalNetRevenuesIFRS` を売上候補に追加。`fin-v7`: US-GAAP 自己株式取得の絶対値） |
