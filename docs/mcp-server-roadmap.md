...
- [ ] LLM によるセクション別売上の構造化抽出：有価証券報告書のセグメント情報（事業別・地域別売上など）は XBRL タグが企業ごとに異なりルールベース抽出が困難なため、LLM を用いてテキスト／テーブルから構造化データへ変換する。`get_filing_content` で取得した本文を入力とし、サーバー側で Anthropic API を呼び出して正規化されたセグメント売上オブジェクトを返す MCP ツール（仮: `get_segment_revenue`）を想定。
