...
- 売上分母は現状 `company_financials` 依存 → statement 正本へ寄せる。employees / rd の Summary は statement に無ければ breakdown 合計。

## 残課題

- 売上分母の financials 逆依存解消（employees / rd は statement に無ければ breakdown）
