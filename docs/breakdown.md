...
- 売上分母は現状 `company_financials` 依存 → statement 正本へ寄せる。employees / rd の Summary 正本は breakdown 分母。

## 残課題

- 売上分母の financials 逆依存解消（employees / rd の Summary 正本は breakdown 分母。ingest の逆依存は残）
