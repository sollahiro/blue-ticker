...
- 売上分母は現状 `company_financials` 依存 → statement 正本へ寄せる。employees / rd の Summary 水準値は breakdown 各軸の合計が正本。

## 残課題

- 売上分母の financials 逆依存解消（employees / rd は breakdown が Summary 正本）
