...
| 銘柄ヘッダ | ツールバーに B マークは出さない。戻るは標準の Liquid Glass。`探す` は出さない。右上は同一グループのアイコン。保有情報は `square.and.pencil`、リストは未追加 `star` / 追加済み `star.fill`（プラスは使わない。塗り分けやチップ色は付けない。VoiceOver は「保有情報」「リストに追加」「リストから削除」）。ヘッダは標準の戻るの右に会社アイコン＋社名（Headline +2pt）をツールバー左のピルに置く。ピルは右上グループ手前までの残幅いっぱいに広げ、アイコンは内側余白を確保する。社名は1〜2行、入りきらない部分は…で切り詰める。コードと業種は出さない |
