...
| 銘柄ヘッダ | ツールバーに B マークは出さない。戻るは標準の Liquid Glass。`探す` は出さない。右上は同一グループのアイコン。保有情報は `square.and.pencil`、リストは未追加 `star` / 追加済み `star.fill`（プラスは使わない。塗り分けやチップ色は付けない。VoiceOver は「保有情報」「リストに追加」「リストから削除」）。会社アイコン＋社名は `.title` 領域の位置に Liquid Glass カプセルを置く（戻ると右上グループの配置をシステムが先に決めて残りを割り当てるので、右上が「…」へ折りたたまれることはない）。見た目のピルはナビのタイトル遷移から外し、銘柄面への push に合わせて同じ位置へフェードインする（切り替わり完了時にはほぼ出ている。横スライドしない。わずかに下から浮かぶ）。銘柄面を離れるときは遷移の序盤でフェードアウトする。戻ると右上は標準のナビのまま。カプセルは短い社名では内容幅に収まり、長い社名は戻ると右のカプセルのあいだまで広げる（被せない。間を空ける）。1行で入る社名の高さは両端のカプセルに揃える。アイコン＋社名はピル内で左寄せ、2行の社名も左揃え。アイコンは内側余白を広めに確保する。社名は1行なら Headline +2pt、収まらないときは時計程度の小さめ（15pt semibold）で最大2行・行間を詰め、入りきらない部分は…で切り詰める。伸縮中は字間を組み直さず、はみ出した字は枠で隠す。収まった枠に入る社名は省略しない。2行のときはカードへ広げてもその2行のままにし、ピルで…になった続きを広がった幅で足す（1行には戻さず、字も縮めない）。ピルの横幅は戻ると右上のカプセルと重ならない上限に抑える。社名ピルをタップすると、そのガラスが上端を保ったまま広がり、社名・銘柄コード・業種・Overview のカードになる。広がったカードではアイコンと社名の上端を、外側の角丸からアイコンの角丸を引いた余白まで下げる（左右の本文余白はピルと同じ）。閉じる `xmark` はその右余白の内側に置く。外タップと `xmark` では、同じガラスがピルの矩形へ戻る。銘柄面を離れたら窓上のガラスは外す（戻る、タブ、別画面への push のあとには残さない）。ピルとカードは別オブジェクトを重ねない。広がっているあいだツールバー側のカプセルは置かない（残すとガラス越しにピルが映る）。ピル自体にはコードと業種は出さない |
...
| 条件 | Screen。業種チップ + 6 プリセット。DualRangeSlider は出さない |
...
| 画面 | Feature | 備考 |
|---|---|---|
| 名称検索 | Feed Update / Search | 「最近新しい有報がアップロードされました」＋キーワード検索＋履歴。Feed 読み込み中は見出し右端に `ProgressView`。取れたらタブ往復では取り直さない（失敗・cancel は次の表示で再試行）。Feed は REST 省略時どおり直近90日を最大10件。同日過多のサンプルはサーバー。Feed Trend（「最近よく調べられています」）は呼び出しを保留中。再開時の 503 は空リスト |
| 条件検索 | Screen（BLT-49） | 業種チップ + 6 プリセット。タップで `GET /v1/screen` の結果へ。全社 `financials` をクライアントで絞らない。右上履歴は名称検索と同じ |
| リスト | （クライアント） | ウォッチリスト 1 本。見出しは「あなたの追加した企業」。行はウォッチ／保有。`pencil` / `完了` + `onMove` で並べ替え。保有入力は銘柄の `保有情報` |
| ファンド | （クライアント） | 「保有株数に応じた業績」と「あなたの保有している企業」（ウォッチは出さない）。保有入力は銘柄の `保有情報`。並べ替えはリストの `pencil` / `完了`。指標は最新 FY Summary の `eps` / `bps`（円/株）。portfolio REST は足さない |
...
- 業種は横スクロール 3 段のチップで複数選択。セクション見出しは「業種を選ぶ」。各段は自然幅で敷き詰める。楕円。選択時は緑枠・薄緑地・緑文字、非選択は一律グレー地の白抜き。見切れマスクの半径はセクション枠の半径から内側オフセットを引く（outer r = inner r + padding）。リスト行の業種タグも選択時と同じ緑枠スタイル。市場チップは出さない。未選択と全選択は `sector` を送らない。1 業種は `sector=` 完全一致。2 業種以上は AND にせず、カンマ区切りの `sector=` 1 リクエストでサーバーの IN 検索（OR）に任せる（ソート・件数もサーバー側）
- DualRangeSlider・指標の詳細トグル・`ScreenMetricFilter` 組み立て UI は出さない。フローは業種（任意）→ プリセットタップ → 結果 → 銘柄
- プリセットは 6 つ。セクション見出しは「こんな企業を探す」（件数の読み込み中は見出し右端に `ProgressView`）。行はラベル（優良=青 / 成長=橙 / 安定=緑 / 高CF=ティール / 改善=ゴールド / 高還元=パープル。業種タグと同形の枠）+ 1 行の説明文（「高収益で財務が健全な企業」など 15 字前後、1 行に収める）。右端に該当件数（`GET /v1/screen` の `matched`。`limit=1`。業種変更から 400ms debounce。6 件揃ったらタブ往復では取り直さない（欠けたら次の表示で再試行）。未選択・全選択は 6 リクエスト。9 業種以上の部分選択は件数を出さない）。クライアントが `GET /v1/screen` の min/max + `sort` desc に写す（閾値は整数、ネット D/E は 1 桁。ソートはプリセットごとにフィルタで必ず非 null になる指標を選ぶ — 高還元は `payout_ratio`、それ以外（改善を含む）は `roic`。ソート指標が null の行はサーバーが落とすため。改善を `roic_cagr_3y` で並べると上位 10 件中 9 件が 3 期前 ROIC < 0 の赤字回復銘柄になる（2026-09 本番相当データ。`roic` 順では上位 50 件中 9 件）ため、変化幅は条件だけに使う）:
...
- 結果行の指標は会社アイコンの左端から業種タグの右端まで使う（社名左端には揃えない）。短い 4 指標（優良・成長）は 1 行。項目名・数値が大きい安定・高CF・改善は 2×2 のまま。高還元は上段に配当性向3年平均と 3年推移（項目名と数値は同一行）、下段に営業利益率 / ROIC / ネットD/E。欠測は `—`（CAGR が null でも YoY に落とさない）:
...
- アカウント設定（有料・ウォッチ同期が要るときだけ）。免責・プライバシー・利用規約は App Store 用の公開ページ。バージョンはファンドフッター。開発ラボは Debug 限定
