...
- プリセットは 3 つ。セクション見出しは「こんな企業を探す」。行はラベル（優良 / 成長 / 安定。業種タグと同じ緑枠）+ 1 行の説明文（「高収益で財務が健全な企業」など 15 字前後、1 行に収める）。クライアントが `GET /v1/screen` の min/max + `sort=roic` desc に写す（閾値は整数、ネット D/E は 1 桁）:
