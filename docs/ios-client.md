...
| 概要 | Summary の水準値（損益など）。年度は古い順に左から右。単位は PL 系・キャッシュ系それぞれで表全体に共通のものを自動選択し、行ラベルに出す。`ROIC` / `ROE` は水準より推移が要るので概要には置かず分解へ回す。銘柄の短い会社説明（Overview）の入力は有報「企業の概況」の「事業の内容」。系統図だけで読めないときは「セグメント情報」の「報告セグメントの概要」へフォールバックする。長さは 50〜80 字が目安で、情報量が少なければ無理に足さない。「Nつの報告セグメントで〜」の枠まとめは書かない。生成・検証は `BlueTickerCore`。格納契約は `company_overviews`（会社1社=1行。由来の有報は `doc_id`。Filing texts キーは増やさない）。ingest stage は `overviews`。公開 REST / iOS 製品面は未配線。 |
