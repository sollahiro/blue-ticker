...
投資に詳しくない人向け。棒の色と符号は「その要因が前年差をどちらへ動かしたか」だけを示す。行を選ぶと次の文言を出す。計算式は現行どおりその下に置く。粗利率や利益率がマイナスの年は、売上・回転・レバレッジの増減と寄与の符号が逆になるので、そのときだけ「変化が押し上げ／押し下げ」と書き、逆転の一文を足す。
