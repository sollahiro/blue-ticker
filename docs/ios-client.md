...
- 結果行の指標は会社アイコンの左端から業種タグの右端まで使う（社名左端には揃えない）。短い 4 指標（優良・成長）は 1 行。項目名・数値が大きい安定・高CF・改善は 2×2 のまま。高還元は上段に配当性向3年平均と 3年推移の 2 行、下段に営業利益率 / ROIC / ネットD/E。欠測は `—`（CAGR が null でも YoY に落とさない）:
  ...
  - **高還元**: 上段は他指標と同じく項目名グレー・数値色付きの 2 行（`配当性向3年平均` / `3年推移` の `XX%→XX%→XX%`。高低の優劣は付けないため色帯なし、数値は accent。推移は大きい Dynamic Type では折り返す）/ 下段 営業利益率 / ROIC / ネットD/E
