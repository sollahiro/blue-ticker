...
| 銘柄ヘッダ | ツールバーに B マークは出さない。戻るは標準の Liquid Glass。`探す` は出さない。右上は同一グループのアイコン。保有情報は `square.and.pencil`、リストは未追加 `star` / 追加済み `star.fill`（プラスは使わない。塗り分けやチップ色は付けない。VoiceOver は「保有情報」「リストに追加」「リストから削除」）。会社アイコン＋社名は `.title` 領域の Liquid Glass カプセルに置く（戻ると右上グループの配置をシステムが先に決めて残りを割り当てるので、右上が「…」へ折りたたまれることはない）。カプセルは短い社名では内容幅に収まり、長い社名ではタイトル領域いっぱいに広がる。アイコン＋社名はピル内で左寄せ、2行の社名も左揃え。アイコンは内側余白を広めに確保する。社名は1行なら Headline +2pt、収まらないときは時計程度の小さめ（15pt semibold）で最大2行・行間を詰め、入りきらない部分は…で切り詰める。伸縮中は字間を組み直さず、はみ出した字は枠で隠す。収まった枠に入る社名は省略しない。2行のときはカードへ広げてもその2行のままにし、ピルで…になった続きを広がった幅で足す（1行には戻さず、字も縮めない）。ピルの横幅は右上カプセルと重ならない上限に抑える。社名ピルをタップすると、そのガラスが上端を保ったまま広がり、社名・銘柄コード・業種・Overview のカードになる。広がったカードではアイコンと社名の上端を、外側の角丸からアイコンの角丸を引いた余白まで下げる（左右の本文余白はピルと同じ）。閉じる `xmark` はその右余白の内側に置く。外タップと `xmark` では、同じガラスがピルの矩形へ戻る。銘柄面を離れたら窓上のガラスは外す（戻る、タブ、別画面への push のあとには残さない）。ピルとカードは別オブジェクトを重ねない。広がっているあいだツールバー側のカプセルは置かない（残すとガラス越しにピルが映る）。ピル自体にはコードと業種は出さない |
