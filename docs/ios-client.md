...
| 概要 | Summary の水準値（損益など）。年度は古い順に左から右。単位は PL 系・キャッシュ系それぞれで表全体に共通のものを自動選択し、行ラベルに出す。`ROIC` / `ROE` は水準より推移が要るので概要には置かず分解へ回す。銘柄の短い会社説明（Overview）は方向のみ。入力は有報「企業の概況」の「事業の内容」。製品面は空枠（Filing texts キーは増やさない）。品質・コストのスパイクは `smoke/overview_mock.html`（公開 REST ではない） |
