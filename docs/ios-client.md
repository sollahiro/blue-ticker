...
| キーワード検索 | 名称検索の `.searchable`。`GET /v1/companies?q=` |
...
| 条件検索 | Screen（BLT-49） | 横断フィルタ UI。Screen REST は未接続。結果は空状態。全社 `financials` をクライアントで絞らない |
| リスト | （クライアント） | ウォッチリスト |
...
| インタビュー | Report（構想） | 本来クライアント責務。v1 はページ枠 |
| レポート | Filing または外部ニュース | ニュースは未決。v1 はページ枠 |
...
- 数値指標は次の 6 つ。各指標は DualRangeSlider（下限・上限、`[minValue, maxValue]`、値はハンドル上）。ソートは `roic` 降順、LIMIT 50 で固定
  ...
  - ROE `roe`
  - ネット D/E `net_de`
- スライダーのハンドル色は水準帯を赤→黄→緑で表す（文言ラベルは出さない）。Screen REST の許可リストは変えない
- 条件の実行はツールバーの `検索`。Screen REST 未接続時は空状態。`絞り込む` は置かない
