...
| 概要 | Summary の水準値（損益など）。年度は古い順に左から右。単位は PL 系・キャッシュ系それぞれで表全体に共通のものを自動選択し、行ラベルに出す。`ROIC` / `ROE` は水準より推移が要るので概要には置かず分解へ回す。銘柄の短い会社説明（Overview）の入力は有報「企業の概況」の「事業の内容」。長さは 50〜80 字が目安で、情報量が少なければ無理に足さない。生成・検証は `BlueTickerCore`（公開 REST / 格納 / iOS 製品面は未配線。Filing texts キーは増やさない）。品質スパイクは `smoke/overview_mock.html` |
