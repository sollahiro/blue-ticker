...
**Region `EU` · Source `ESEF` の方針・フェーズ・非ゴール**。命名は `.agents/rules/regions.md`。構成は `architecture.md`。JP の方針は `blt-server-roadmap.md`。Summary 組立は `financials-summary-separation.md`（JP と同じ形を目指す）。進捗と未決の現在地は Linear（[EU 現在地](https://linear.app/sollahiro/document/eu-現在地-844f7112eb70)）。
...
- Summary は水準値投影。Waterfall は同行走査の分析投影。
...
- **REST `/v1` が契約の正**（EU 公開時も同じ）。MCP は開発専用で、製品として追従させない。
...
ここは EU 固有の方針のみ。進捗は Linear（[EU 現在地](https://linear.app/sollahiro/document/eu-現在地-844f7112eb70)）。

| Class | Feature | EU 固有の方針 |
|---|---|---|
| Meta | Search | identity は LEI / `identifier` / `fxo_id` / 名称の完全一致。部分一致・全件 index は ESAP 後 |
| Meta | Icon | 当面保留 |
| Struct | Filing | セクション方針は Linear 未決 |
...
`architecture.md` · `financials-summary-separation.md` · `blt-server-roadmap.md` · `scripts/eu/esef/README.md` · `.agents/rules/regions.md`
