...
3. ~~**追加ソースの採用範囲**~~（2026-07-19 確定・実装済み）: US-GAAP 企業（キヤノン）は `segments` が実は `method == "html_table"` で注23の事業別セグメント表（外部顧客向け行）を既に正しく抽出できていた（07-19 の `SegmentExtractor` 前処理修正の副産物。当時の学び9記述は古い調査時点の情報で現状と食い違っていたため本項で更新）。オークマ型（`segments` の axis が geography 判定）は収益認識注記（`NotesRevenueRecognitionConsolidatedFinancialStatementsTextBlock`、`SegmentExtractor.extractRevenueRecognitionInfo` で新規抽出）から製品別内訳を拾う。
   ...
   - **利益（profit）も対応済み（2026-07-19追記）**: `xbrl_facts` 経路（学び参照）と同様、business 軸の LLM 経路（`html_table_llm`/`revenue_recognition_llm`）でも任意フィールドとして拾う。jsonSchema に `profit`（nullable number, required だが null 許容）を追加し、business プロンプトに「売上と同じ表に事業別営業利益が並んでいれば転置して拾う。無ければ null」と明示。geography プロンプトは「profit は常に null」で対象外のまま固定（コード側でも axis="geography" は profit を常に nil にクランプし、プロンプト任せにしない。Fable監査指摘）
     - **「未開示（確認済み）」と「LLMの見落とし」の区別（同日追記2、ユーザー指摘で対応）**: `profit == nil` だけでは両者を区別できないため、LLM に `profit_disclosed`（その表に事業別利益情報が存在するかの自己申告、bool）も返させ、`LLMBreakdownAudit`/`LLMBreakdownAuditPayload` に保持する。rows の実際の profit 値との矛盾（`profit_disclosed=true` なのに1行も profit が無い／`false` なのに値がある）は決定的ガードで検知し `needs_review` + `profit_disclosed_but_row_missing` / `profit_present_despite_not_disclosed` を立てる。`profit_disclosed` キー自体が欠落・型不正な場合も silent に false（＝確認済み未開示）扱いせず `llm_profit_disclosed_unresolved` で「不明」を明示する（Fable監査指摘。`unit` の "other" フラグ付けと同じ考え方）
     - 実データ確認済み: キヤノンは注23表に営業利益行があるため `profit_disclosed=true` で正しく拾得（プリンティング255,759／メディカル32,775／イメージング172,871／インダストリアル62,525／その他及び全社△69,451、百万円、needsReview=false）、オークマは収益認識注記に利益列が無いため `profit_disclosed=false`・全行 profit=null（needsReview=false、「未開示」が確認済みの状態として正しく成立）。`smoke/segment_breakdown_business_expected.json` に反映済み（profitキーの有無がそのまま開示有無を表す）
