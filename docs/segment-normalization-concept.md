...
6. **LLM の位置づけ** — 本番経路に載せるか、候補生成・評価のみか（費用・再現性・監査）
...
| 信頼フラグ付き保存 | LLM（または軸判定）出力に `confidence` / `needs_review` / `warnings` を必須化。再計算・要確認は **フラグ付き・低 confidence・検証失敗** だけに限定（正しい行は据え置き） |
