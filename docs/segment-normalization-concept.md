...
4. **`SegmentExtractor` の前処理欠陥** — period 誤ラベル・US-GAAP 巨大注記未対応・geography への表混入は解消済み（別 worktree `worktree-fix-segment-extractor-defects`、main 未マージ）。残るのは巨大注記内の見出し・テーブル意味的関連性の構造的限界のみ
   - **キヤノン（S100XTLJ）の具体例（2026-07-19、LLM正規化契約検証で発覚・ユーザー確認済み）**: `geography` の候補表は2枚のみで、1枚目は事業別の収益認識タイミング表（地域別ではない）、2枚目が地域別売上表だが period ラベルは "当期" でも**実際の中身は前年度（FY2024-12）のデータ**。連結売上高（FY2025-12、円）との差は約2.48%で、分母整合性チェック（許容 0.90〜1.10、xbrl_facts 経路の 0.95〜1.05 より広め）の範囲内に収まってしまうため、**この種の「年度自体が違う」誤りは分母合計チェックでは原理上検知できない**（前年度と当期の地域構成比は通常近いため、金額の乖離が小さく出る）。LLM 自身は notes に「唯一の候補表のため分母不一致を承知の上で採用」と明記しており、目視監査（audit 情報の表示）で発見できた。根本原因は `SegmentExtractor` の period 判定（この文書には真の当期地域別表がそもそも存在しない可能性を含む）で、上記の別ブランチのfixが本当にこのケースを解消するかは未検証。**次にゴールデン fixture 化する際は、この上流 bug の解消を確認するまでキヤノンを候補から除外する**
