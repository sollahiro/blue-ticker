...
| 4911 | 資生堂 | **空（F）** | 2026-07-25 に business 行 6 件 DELETE。live は not_found |
...
- 資生堂は **F**（製品別省略 → business 空）。「ギャップ埋め」は **DB を仕様（空）に揃える**の意味（地域事業ユニットを business 採用する話ではない）。誤って business 採用＋ingest した行は 2026-07-25 に DELETE 済み

### 空き（空のままでよい）

| 分類 | コード |
|------|--------|
| **F** | 3436, 4704, 6273, 6526, 6532, 8697, 6861, 7186, 8331, 8354, 4507, 4519, 5214, 3092, **4911**（DB 空） |
...
2. v5 後に日経225全件（または limit 付き）Stage 6 再 ingest（4911 は live not_found のため行は再作成されない想定）
3. Secret の `DATABASE_URL` を direct（非 pooler）に揃えると書き込み手順が単純になる
