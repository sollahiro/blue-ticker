...
### 案 A: Swift CLI ラッパー＋Python コア維持（非推奨）

Python コアを維持しつつ、Swift で薄い CLI ラッパーを作る構成。

**問題点**:
- Swift ↔ Python の境界（subprocess IPC または Python C API）がそれ自体の複雑さを生む
- Python ランタイムを同梱しない限り「単一バイナリ」にならない——配布簡素化という動機が達成できない
- 型安全性・パフォーマンスなど Swift 移行のメリットがコア層に及ばない

**結論**: コストを払ってもメリットが得られない中途半端な構成のため除外。
...
Phase 2: HTTP クライアント + サービス層 + データ集計
...
全レイヤーを同時に Swift 化。型安全性と配布最適化を最大化できるが、動作確認できる中間状態がなくリスクが最も高い。
...
**推奨: 案 B（段階移行）**。CLI とインフラから着手することで早期に真の単一バイナリを得ながら、最大リスクの XBRL 解析を既存 Python 版と並走させて安全に移植できる。

XBRL 解析の移植は既存 Smoke テスト（10 社）をゴールデンファイルとして用い、モジュール単位で Python 版の出力と一致させながら進めることで品質を担保する。
